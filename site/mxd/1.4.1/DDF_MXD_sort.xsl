...
<xsl:stylesheet xmlns:xsl="http://www.w3.org/1999/XSL/Transform" xmlns:mx="http://mx.forskningsdatabasen.dk/ns/documents/1.4" version="1.0">
    ...
    <xsl:template match="/mx:ddf_doc">
        <xsl:copy>
            ...
            <xsl:apply-templates select="@total_authors"/>
            ...
            <xsl:apply-templates select="mx:related_object"/>
        </xsl:copy>
    </xsl:template>
    <xsl:template match="/mx:ddf_doc/mx:description">
        <xsl:copy>
            ...
            <xsl:apply-templates select="mx:identifier"/>
        </xsl:copy>
    </xsl:template>
    ...
    <xsl:template match="/mx:ddf_doc/mx:description/mx:identifier">
        <xsl:copy>
    ...
    <xsl:template match="/mx:ddf_doc/mx:publication/mx:other">
        <xsl:copy>
            <xsl:apply-templates select="@xml:lang"/>
            <xsl:apply-templates select="text()"/>
            <xsl:apply-templates select="mx:text"/>
    ...
    <xsl:template match="/mx:ddf_doc/mx:related_object">
        <xsl:copy>
            <xsl:apply-templates select="@managed"/>
            <xsl:apply-templates select="@rel_type"/>
            <xsl:apply-templates select="@obj_type"/>
            <xsl:apply-templates select="@xml:lang"/>
            <xsl:apply-templates select="text()"/>
        </xsl:copy>
    </xsl:template>
